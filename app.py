from flask import Flask, requestimport osimport requestsimport uuidimport timeimport random

from datetime import datetime, timedelta, timezone

from linebot.v3 import WebhookHandlerfrom linebot.v3.messaging import (Configuration,ApiClient,MessagingApi,ReplyMessageRequest,PushMessageRequest,TextMessage,ImageMessage,StickerMessage)

from linebot.v3.webhooks import (MessageEvent,TextMessageContent,StickerMessageContent,ImageMessageContent,AudioMessageContent,VideoMessageContent)

from linebot.exceptions import InvalidSignatureErrorfrom supabase import create_client
...
nickname_1 = ["星", "月", "白", "夜", "風","雨", "雪", "海", "雲", "光","影", "花", "羽", "夢", "霧"]

nickname_2 = ["空", "辰", "羽", "夜", "風","語", "海", "光", "夢", "森","月", "雪", "櫻", "川", "歌"]
...
CHANNEL_ACCESS_TOKEN = os.getenv("CHANNEL_ACCESS_TOKEN")CHANNEL_SECRET = os.getenv("CHANNEL_SECRET")
...
@handler.add(MessageEvent, message=TextMessageContent)def handle_text(event):
...
@handler.add(MessageEvent, message=StickerMessageContent)def handle_sticker(event):
...
@handler.add(MessageEvent, message=ImageMessageContent)def handle_image(event):
